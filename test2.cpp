...
{
     item[x].totalSales = item[x].PperKg * item[x].weight * item[x].quantity;
...
void sortGrocery(Groceries item[],int S)
{
    ...
    for (int x = 0;x < S ; x++)
    {
        ...
        << setw(10)<<right << item[x].PperKg
        << setw(15)<<right <<item[x].weight
        << setw(15)<<right <<item[x].quantity
        << setw(15)<<right <<item[x].totalSales <<"\n";
